...
function register(req, res, next){
    ...
    if(fullName.mach(fullNameRegex)){
    ...
    if(mail.mach(mailRegex)){
        return res.status(400).send({alert:["Informe E-mail válido"]})
    }

    if(address == null || address == ""){
        return res.status(400).send({alert:["O campo endereço é obrigatório"]})
    }
